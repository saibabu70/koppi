...
// producer thread waits while list is full while (list.size()==capacity)
wait();
...
synchronized (this)
{
    // consumer thread waits while list is empty while (list.size()==0)
    wait();
    //to retrive the first job in the list int val = list.removeFirst();
